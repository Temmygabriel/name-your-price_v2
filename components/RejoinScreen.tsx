"use client";
// Name Your Price — Rejoin / Check Game Screen v1.0
// Identical to HTP RejoinScreen. Only status strings and colour tokens differ.
...
const STATUS_LABELS: Record<string, { label: string; color: string; action: string }> = {
  lobby:     { label: "In lobby",         color: "#00D4FF", action: "Rejoin Lobby"  },
  voting_1:  { label: "Round 1 — Live",   color: "#00FF87", action: "Rejoin Game"   },
  voting_2:  { label: "Round 2 — Live",   color: "#00FF87", action: "Rejoin Game"   },
  voting_3:  { label: "Round 3 — Live",   color: "#00FF87", action: "Rejoin Game"   },
  judging:   { label: "AI Judging...",    color: "#A78BFA", action: "Watch Results" },
  completed: { label: "Game over",        color: "#888899", action: "View Results"  },
};
...
export default function RejoinScreen({ playerAddress, onRejoin, onBack }: RejoinProps) {
  ...
  const statusInfo = room
    ? STATUS_LABELS[room.status] || STATUS_LABELS["completed"]
    : null;
  ...
  return (
    <div className="screen">
      ...
      <h2 className="screen-title">Check Game</h2>
      ...
      {state === "found" && room && statusInfo && (
        <div className="room-result">
          <div className="room-result-header">
            ...
            <span className="room-result-status" style={{ color: statusInfo.color }}>
              {statusInfo.label}
            </span>
          </div>
          ...
          <button className="btn-primary" onClick={() => onRejoin(room, room.code)}>
            {statusInfo.action}
          </button>
        </div>
      )}
      ...
      <div className="rejoin-tip">
        <strong>Tip:</strong> Results are stored on-chain permanently. You can always
        come back to check them using your room code.
      </div>
    </div>
  );
}
